...
public class ShowMsg {
    ...
    public static void showResult(String method, int code, String errMsg, Context context) {
        String msg = "";
        if (errMsg.isEmpty()) {
            msg = String.format(
                    "\t%s\n%s\t%s\t%s\n",
    ...
    private static String getCodeText(int state) {
        ...
        switch (state) {
            ...
            case Epos2CallbackCode.CODE_BUSY:
                return_text = "BUSY";
                break;
            case Epos2CallbackCode.CODE_DISCREPANCY:
                return_text = "DISCREPANCY";
                break;
            case Epos2CallbackCode.CODE_ERR_CASH_IN_TRAY:
                return_text = "CASH_IN_TRAY";
                break;
            case Epos2CallbackCode.CODE_ERR_SHORTAGE:
                return_text = "ERR_SHORTAGE";
                break;
            case Epos2CallbackCode.CODE_ERR_REJECT_UNIT:
                return_text = "ERR_REJECT_UNIT";
                break;
            case Epos2CallbackCode.CODE_ERR_OPOSCODE:
                return_text = "ERR_OPOSCODE";
                break;
            case Epos2CallbackCode.CODE_ERR_UNSUPPORTED:
                return_text = "ERR_UNSUPPORTED";
                break;
            case Epos2CallbackCode.CODE_ERR_PARAM:
                return_text = "ERR_PARAM";
                break;
            case Epos2CallbackCode.CODE_ERR_COMMAND:
                return_text = "ERR_COMMAND";
                break;
    ...
    private static String getConnectionEventText(int eventType) {
        String return_text = "";
        switch (eventType) {
            case CashChangerFragment.EVENT_RECONNECTING:
                return_text = "EVENT_RECONNECTING";
                break;
            case CashChangerFragment.EVENT_RECONNECT:
                return_text = "EVENT_RECONNECT";
                break;
            case CashChangerFragment.EVENT_DISCONNECT:

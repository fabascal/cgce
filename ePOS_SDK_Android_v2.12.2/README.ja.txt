=========================================================================
          Epson ePOS SDK for Android  Version 2.12.2
...
・以下のクラスのイベントでは通知されるパラメータがNULLになっている可能性があります。
  アプリケーションはNULLチェックしてから使用してください。

  CashChangerクラス
    onCChangerCashCount、dataパラメータ
    onCChangerDeposit、dataパラメータ
  CATクラス
    onCatAuthorizeSales、resultパラメータ
    onCatAuthorizeVoid、resultパラメータ
    onCatAuthorizeRefund、resultパラメータ
    onCatAuthorizeCompletion、resultパラメータ
    onCatAccessDailyLog、dailyLogパラメータ

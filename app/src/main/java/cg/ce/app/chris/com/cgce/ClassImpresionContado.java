...
public class ClassImpresionContado extends AsyncTask<JSONObject,String,Boolean> implements ReceiveListener {
    ...
    public ClassImpresionContado (ticketFragment activity, Context context, ImageButton btn, JSONObject jsonObject){
        ...
        if (!ticket.has("validador")) {
            pdLoading = new ProgressDialog(activity.getActivity());
        }
        try {
    ...
    public boolean createReceiptData() {
        String method = "";
        Bitmap logoData = BitmapFactory.decodeResource(activity.getResources(), R.drawable.logobienvenidaener);
